...
This section summarizes the key findings from the league-specific regression analyses, emphasizing only significant predictors of market value. Insignificant estimates, such as national team rankings and most position-specific coefficients, are excluded from this section but are included in @sec-full.
...
ggplot(assists_data, aes(x = League, y = Estimate, fill = League)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = scales::label_number(accuracy = 0.001)(Estimate)), 
            vjust = -0.25, size = 3) +
  scale_fill_manual(values = c(
    "England" = "#D62728",
    "France" = "#1F77B4",
...
![Example to Retrieve National Team Ranking Data](../other/photos/National_Team_Ranking.png){#fig-national_team_ranking fig-pos="H"}
...
![Example to Retrieve Club Ranking Data](../other/photos/Club_Ranking.png){#fig-club_ranking fig-pos="H"}
...
## Detailed Model Results{#sec-full}
